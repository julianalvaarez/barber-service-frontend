...
export function Agenda() {
  const {bookings, loadingGeneral, loadBookings, barbers, loadBarbers, generateAgendaForDay, slotList, selectedBarber, setSelectedBarber} = useBarberContext();
  ...
  useEffect(() => {
    generateAgendaForDay(date, selectedBarber)
      ...
      loadingGeneral ?
        ...
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8 ">
            {slotList.map((slot) => (
                <div key={slot.hour} className={`p-3 rounded-lg border ${!slot.availability ? "bg-red-50 border-red-200" : "bg-green-50 border-green-200"}  ${!slot.availability ? "dark:bg-red-200 dark:border-red-200" : " dark:border-green-900"}` }>
        ...
        {openBooking && <BookingDialog booking={openBooking} onClose={() => { setOpenBooking(null) }} onSave={() => {loadBookings(); setOpenBooking(null);  }} />}
        </Card>
